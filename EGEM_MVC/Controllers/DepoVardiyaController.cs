...
{
    public class DepoVardiyaController : Controller
    {
        ...
        public ActionResult DepoVar(int? Page_No)
        {
            var students = from stu in db.EGEM_DEPO_VARDIYA select stu;
            students = students.OrderByDescending(stu => stu.ID);
            int Size_Of_Page = 10;
            int No_Of_Page = (Page_No ?? 1);
            return View(students.ToPagedList(No_Of_Page, Size_Of_Page));
        }
        public ActionResult DepoVardetay(int? id)
        {
            List<EGEM_DEPO_VARDIYA> studentList = db.EGEM_DEPO_VARDIYA.ToList();
